...
- **Página 4 (Oportunidades):** Matriz cruzando Faixa de Renda (linhas) × Classificação (colunas), com o score médio de utilização de produtos (`INDICE_CLASSIFICACAO`) como valor — quanto menor o score, maior a oportunidade de negócio (renda alta associada a baixa utilização/diversificação de produtos). A matriz funciona como filtro visual (interação nativa do Power BI) para uma tabela detalhada de associados, ordenada de forma ascendente pelo mesmo score, trazendo também os pilares `SCORE_*` individuais como informação auxiliar de composição. As três flags de oportunidade da seção 6 continuam calculadas na Gold e disponíveis (`FLAG_OPORTUNIDADE_*`, medidas de contagem em `_Medidas`), mas a exploração por matriz/ranking passou a ser o mecanismo principal da página.

Nenhuma das outras três páginas usa os campos intermediários de cálculo (níveis individuais de movimentação, colunas de produto por tipo, flags `*_INVALIDO`); a Página 4 é exceção deliberada, consumindo `INDICE_CLASSIFICACAO` e os pilares `SCORE_*` para ranking. Por isso o import recomendado no Power BI é `data/2_gold/features_dashboard.parquet`, a projeção de `features.parquet` só com as colunas necessárias às 4 páginas (`build_dashboard_features`, `src/features/consolidado.py`; lista completa em `dicionario_dados.md`, seção 5.6), relacionada às cinco dimensões por ID (`dim_*.parquet`, seção 6 de `dicionario_dados.md`), à `dim_calendario.parquet` (seção 6.6, relacionada por data via `DATA_ASSOCIACAO`) e à `dim_agencia.parquet` (seção 6.7, relacionada por `AGENCIA`) em modelo estrela.
